...
function OverviewTip({ active, payload, label }: { active?: boolean; payload?: { name: string; value: number; color: string }[]; label?: string }) {
  ...
  return (
    <div className="rw-tip">
      <div className="rw-tip-h">{label} 2026</div>
      {rows.map((p) => (
...
function BandDot(props: { cx?: number; cy?: number; payload?: RiwayatRun }) {
  ...
  return <circle cx={cx} cy={cy} r={4.5} fill={SIG[payload.cls]} stroke="var(--surface)" strokeWidth={1.5} />;
}

const AXIS = { fill: "var(--dim)", fontSize: 11 };

export default function RiwayatDashboard({ series, summary }: { series: RiwayatSeries[]; summary: RiwayatSummary }) {
  const [sel, setSel] = useState<string>(series[0]?.symbol ?? "ALL");
  const current = series.find((s) => s.symbol === sel) ?? null;
  ...
  return (
    <div className="rw-dash">
      ...
      {/* filter emiten */}
      <div className="rw-filter" role="tablist" aria-label="Pilih emiten">
        <button role="tab" aria-selected={sel === "ALL"} className={sel === "ALL" ? "on" : ""} onClick={() => setSel("ALL")}>Semua</button>
        {series.map((s) => (
          <button key={s.symbol} role="tab" aria-selected={sel === s.symbol} className={sel === s.symbol ? "on" : ""} onClick={() => setSel(s.symbol)}>
            {s.symbol} <small>{s.runs.length}</small>
          </button>
        ))}
      </div>

      {/* satu chart, mengikuti filter */}
      <section className="rw-panel">
        <div className="rw-panel-h">
          {sel === "ALL" ? (
            ...
            <>
              ...
              <p>{current.runs.length} investigasi · {current.runs[0].date} – {current.runs[current.runs.length - 1].date}. Zona warna = tingkat skor.</p>
            </>
          ) : null}
        </div>
        ...
        <div className="rw-chart">
          <ResponsiveContainer width="100%" height="100%">
            {sel === "ALL" ? (
              <LineChart data={overview} margin={{ top: 8, right: 16, bottom: 4, left: -16 }}>
                <CartesianGrid stroke="var(--border)" vertical={false} />
                <XAxis dataKey="date" tick={AXIS} stroke="var(--border)" tickMargin={8} />
                <YAxis domain={[0, 100]} ticks={[0, 30, 60, 80, 100]} tick={AXIS} stroke="var(--border)" />
              ...
              <LineChart data={current?.runs ?? []} margin={{ top: 8, right: 16, bottom: 4, left: -16 }}>
                ...
                <Line type="monotone" dataKey="score" stroke="var(--accent)" strokeWidth={2.4} dot={<BandDot />} activeDot={{ r: 6 }} isAnimationActive={false} />
              </LineChart>
            )}
          </ResponsiveContainer>
        </div>
        ...
        {sel === "ALL" && (
          <div className="rw-legend">
            {series.map((s, i) => (
              <button key={s.symbol} className="rw-leg" onClick={() => setSel(s.symbol)} title={`Lihat ${s.symbol}`}>
                <span className="rw-leg-d" style={{ background: PALETTE[i % PALETTE.length] }} /> {s.symbol}
              </button>
            ))}
          </div>
        )}
      ...
      {/* detail di bawah chart, untuk emiten terpilih */}
      {current && (
        <section className="rw-detail">
          <div className="rw-detail-h">Detail investigasi {current.symbol}</div>
          <div className="rw-rows">
            {current.runs.map((r, i) => {
              const prev = i > 0 ? current.runs[i - 1].score : null;
