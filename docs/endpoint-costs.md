...
> 9 dari 15 endpoint sudah diukur lewat `make spike` (`spikes/observed.json`, 2026-09-08T10:20:40+00:00). Sisanya masih **asumsi** — lihat kolom *Status*.

## Tier 1 — sapuan market-wide harian

Dijalankan cron tiap hari bursa. Total **6 kredit/hari** (target ≤6 — dijaga `tests/sectors/test_routing.py`).

| Endpoint | Kredit | Status | Transport | Cadangan | Untuk apa |
| --- | ---: | --- | --- | --- | --- |
| `fetch-close` | 1 | asumsi | mcp | rest | Harga penutupan seluruh ticker IDX untuk satu tanggal bursa. |
...
| `fetch-most-traded-stocks` | 2 | terukur | rest | mcp | Saham paling banyak diperdagangkan pada rentang tanggal. |
...
| Endpoint | Kredit | Status | Transport | Cadangan | Untuk apa |
| --- | ---: | --- | --- | --- | --- |
| `fetch-broker-summary` | 1 | terukur | mcp | — | Rincian net buy/sell per kode broker untuk satu ticker. |
| `fetch-broker-summary-top` | 2 | asumsi | mcp | rest | Broker dengan net buy/sell terbesar pada satu ticker dan rentang tanggal. |
...
| `fetch-corporate-actions` | 1 | asumsi | mcp | — | Aksi korporasi: rights issue, stock split, dividen, private placement. |
| `fetch-daily-transaction` | 1 | terukur | rest | mcp | Harga, volume, dan kapitalisasi harian satu ticker pada rentang tanggal. |
| `fetch-foreign-flow` | 1 | terukur | rest | mcp | Arus dana asing bersih harian pada satu ticker. |
| `fetch-free-float` | 1 | asumsi | mcp | — | Persentase saham beredar bebas (free float) per emiten. |
| `fetch-quarterly-financials` | 1 | asumsi | mcp | — | Laporan keuangan kuartalan: pendapatan, laba bersih, total aset. |
| `fetch-shareholders-composition` | 1 | terukur | mcp | — | Komposisi pemegang saham dan porsi kepemilikan asing. |
